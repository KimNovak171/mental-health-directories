      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${cityName}, ${provinceName} mental health directory preview`,
        },
      ],
    },
  };
...
export default async function CanadaCityPage({ params }: CanadaCityPageProps) {
  ...
  const webpageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    name: `Mental Health Services in ${cityName}, ${provinceName}, Canada`,
    ...
    about: [
      { "@type": "Thing", name: `${cityName} mental health services` },
    ...
    <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <header className="space-y-4">
        ...
        <h1 className="text-3xl font-semibold text-navy">
          Mental Health Services in {cityName}, {provinceName}
        </h1>
        <p className="max-w-2xl text-sm text-slate-600">
          {cityName} has {facilities.length.toLocaleString()} verified mental health
          services including {careTypesText}. Browse all options below,
          ...
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
            {otherCities.map((city) => (
              <Link
                ...
                className="rounded-lg border border-surface-muted border-l-[3px] border-l-navy bg-surface px-3 py-2 text-sm text-navy shadow-sm transition hover:border-[#2A7F7F] hover:bg-[#1B3A5C] hover:text-white"
              >
